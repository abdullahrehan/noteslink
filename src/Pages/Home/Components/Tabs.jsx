...
function Tabs({ name, tabsInnerRef, tabs, setTabs, index, id, currentTabIndex, setCurrentTabIndex, tabsRef }) {
    ...
    return (
        ...
        <div onClick={openTab} className={`w-[180px] h-[95%]  flex items-end justify-center transition-all delay-70 duration-500 ease-in-out ${tabType == "prev" ? "bg-white" : tabType == "selected" ? "bg-[#373737]" : tabType == "next" ? "bg-white" : null}`} ref={(element) => tabsRef.current[index] = element}>

            <div onClick={openTab} className={`w-full  flex justify-center text-base center hover:cursor-pointer transition-all delay-70 duration-500 ease-in-out ${tabStyle}  `}  >
